...
# 🚀 AutoDevKit - Interactive Development Environment Setup
...
# Print functions (using printf instead of echo -e for better compatibility)
print_header() {
    printf "${PURPLE}\n"
    printf "╔══════════════════════════════════════════════════════════╗\n"
    printf "║                                                          ║\n"
...
ask_technology() {
    ...
    case $TECH_CHOICE in
        ...
        *) 
            print_error "Invalid choice. Defaulting to general."
            TECHNOLOGY="general"
            ;;
    esac
}
...
add_npm_scripts() {
    print_info "Updating npm scripts..."
    
    # The scripts are already added in create_package_json function
    print_success "npm scripts already configured in package.json!"
}
